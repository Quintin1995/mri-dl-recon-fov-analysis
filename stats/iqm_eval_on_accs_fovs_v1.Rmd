...
```{r boxplot}
# Here we will create a boxlplot that looks really good.
# Create boxplot for each IQM and write to file (current working directory)
# Each boxplot will have the 3x and 6x on the x-axis and the IQM on the y-axis.
# Then we the roi type (should be categorical) should be the color or hue.
# So that in the same plot we have multiple boxplots
# Should we do outlier removal?
# The legend should be outside on the right side of the figure.
# Create images of high resolution
```
### Violin Plot?
```{r violin}
# Similar to the boxplot design
```
